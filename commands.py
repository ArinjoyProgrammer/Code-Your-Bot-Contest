...
class Commands(commands.Cog):
    ...
    async def github(self, ctx):
        embed = discord.Embed(title="Perfectz-Discord-Bot (Github)", description=f"The GitHub page is currently Private\nIt will be public when the bot will reach 50 Discord Servers!\nThe bot is in now in {len(self.client.guilds)} Discord Servers!", timestamp=datetime.utcnow(), color=discord.Color.purple())
        embed.set_footer(text=f"Requested by {ctx.author}")
